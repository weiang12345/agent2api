...
pub struct AtmCodeAdapter;
pub static ATMCODE_ADAPTER: AtmCodeAdapter = AtmCodeAdapter;

impl ProviderAdapter for AtmCodeAdapter {
    ...
    fn build_chat_request(
        ...
        let user_id = account
            ...
            .and_then(Value::as_str)
            .unwrap_or_default();
        chat::build_chat_request(token, user_id, body)
    }
    ...
    fn credentials_expiring(&self, store: &AccountStore, account_id: &str) -> bool {
        store
            .atomcode_account_record(account_id)
...
fn credentials_for(store: &AccountStore, account_id: &str) -> Result<Credentials, GatewayError> {
    let record = store
        .atomcode_account_record(account_id)
...
async fn refresh_if_needed(
    ...
    if let Some(expected) = store.atomcode_account_record(account_id) {
        let _ = store.update_atomcode_credentials_if_current(&expected, &refreshed);
    }
    Ok(refreshed)
}
