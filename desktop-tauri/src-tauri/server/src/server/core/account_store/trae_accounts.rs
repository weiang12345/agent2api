...
impl AccountStore {
    ...
    pub fn add_trae_account(
        ...
        let id = existing
            ...
            .unwrap_or_else(|| {
                format!("trae-{:x}", Sha256::digest(credentials.user_id.as_bytes()))
            });
        ...
        fields.insert(
            ...
            Value::String(credentials.enterprise_id.clone()),
        );
        fields.insert(
            "tokenTail".to_string(),
        ...
        fields.insert(
            "machineId".to_string(),
            Value::String(credentials.machine_id.clone()),
        );
        fields.insert(
            "deviceId".to_string(),
        ...
        logging::log(
            "[Accounts]",
            &format!("✅ Trae 账号已保存（优先级 {priority}）"),
        );
